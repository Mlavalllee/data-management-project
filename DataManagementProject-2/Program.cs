...
void DisplayFavourite(string User)
{
    ...
    for (int i = 0; i < AccountData.Count; i++)
    {
        string account = AccountData[i].ToString();
        string[] accountdata = account.Split(",");
        if (accountdata[i] == User)
        {
            for(int index = 0; index < accountdata.Length; index++)
            {
                if(index != 0 || index != 1)
                {
                    string[] favourites = account.Split(".");
                    for(int favs = 0; favs < favourites.Length; favs++)
                    {
                        if(favs <= favourites.Length - 2)
                        {
                            Console.WriteLine($"Genre: {favourites[favs]} Artist: {favourites[favs + 1]} Title: {favourites[favs + 2]}");
                        }
                    }
